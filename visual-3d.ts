  ...
  private animation() {
    ...
    if (this.starLines) {
      ...
      for (let i = 0; i < 400; i++) {
        ...
        positions[i * 6] += vx * dt * speedMult;
        positions[i * 6 + 1] += vy * dt * speedMult;
        positions[i * 6 + 2] += vz * dt * speedMult;

        positions[i * 6 + 3] = positions[i * 6] - vx * streakLength * 10;
        positions[i * 6 + 4] = positions[i * 6 + 1] - vy * streakLength * 10;
        positions[i * 6 + 5] = positions[i * 6 + 2] - vz * streakLength * 10;
      ...
      (this.starLines.material as THREE.LineBasicMaterial).opacity =
        this.warpFactor * 0.3;
    }
